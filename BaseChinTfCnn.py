...
def readLabelledData():
    ...
    for i, file in enumerate(os.listdir('labelled')):
        ...
        if file.endswith('.pgm'):
            img = binarize(cv2.imread('labelled/' + file, 0))
            utf = file[:4]
...
def createSubData(labelled_data, train_n, test_n, valid_n):
    ...
    valid_set = []
    allclasses = []
    for i, (utf, image) in enumerate(labelled_data):
        if not(utf in allclasses):
            allclasses.append(utf)
        if(i < train_n):
            train_set.append((utf, image))
            continue
        if(i < train_n+test_n):
            test_set.append([utf, image])
            continue
        if(i < train_n+test_n+valid_n):
            valid_set.append([utf, image])
            continue
    return train_set, test_set, valid_set, allclasses
...
def print_test_accuracy(show_example_errors=False,
    ...
    while i < num_test:
        ...
        images = [x[1] for x in test_data[i:j]]
        # print(images[1])
        # Get the associated labels.
        labels = [x[0] for x in test_data[i:j]]
        # print(labels[1])
...
if __name__ =="__main__":
    ...
    test_n = 50
    valid_n = 50
    #train_data, test_data, valid_data, allclasses = readLabelledData(tot_img, train_img, test_img, valid_img)
    labelled_data = readLabelledData()
    shuffle(labelled_data)

    train_data, test_data, valid_data, allclasses = createSubData(labelled_data, train_n, test_n, valid_n)
    print('I divided the data as follows:\nTrain data size: ' + repr(len(train_data)) + '\nTest_data size: ' + repr(len(test_data)) + '\nValid_data size: ' + repr(len(valid_data)))
    ...
    print('True class: ' + repr(cls_true))
